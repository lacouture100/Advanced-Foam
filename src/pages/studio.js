...
export async function getStaticProps( {params} ) {
    ...
    const response1 = await axios.get(`https://www.backend.advancedfoam.com/wp-json/wp/v2/posts?categories=${wpCategoryFilter}`);
    ///console.log("res: " + response)
    const bannerPostList = await response1.data
    const response = await axios.get(`https://www.backend.advancedfoam.com/wp-json/wp/v2/posts?categories=23`);
    ///console.log("res: " + response)
...
Studio.getInitialProps = async () => {
    //Studio projects
    const response = await axios.get(`https://www.advfoam.com/wp-json/wp/v2/posts?categories=3`);
